    ...
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          current_loss = losses_dict['Loss/total_loss'].numpy()

          # Early stopping check. Compare current_loss to best_loss.
          if (best_loss - current_loss) > min_delta:
              ...
              if wait >= patience:
                  tf.logging.info(
                      "Early stopping triggered at step {}. Loss hasn't "
                      "improved in {} consecutive iterations.".format(
                          global_step.value(), patience))
                  break
